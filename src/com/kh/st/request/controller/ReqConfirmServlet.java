...
/**
 * Servlet implementation class ReqConfirmServlet
 */
@WebServlet("/reqConfirm.tn")
public class ReqConfirmServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ReqConfirmServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			String multiTextResult = multiRequest.getParameter("textResult1");	//검수내용
			String multiChecker = multiRequest.getParameter("checker");	//검수자 회원번호
			String multiStatus = multiRequest.getParameter("product");	//물품 상태
			
			System.out.println("요청번호 : " + multiReqNo);
			System.out.println("검수내용: " + multiTextResult);
			System.out.println("검수자 회원 번호   : " + multiChecker);
			System.out.println("물품 상태 : " + multiStatus);
			...
			int result = new CheckHistoryService().insertConfirmImg(ch, fileList);
			...
			if(result > 0) {
				...
				request.setAttribute("msg", "검수 승인 실패");
